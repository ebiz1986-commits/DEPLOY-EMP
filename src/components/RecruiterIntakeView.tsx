...
import { Category, Company, Worker } from "../types";
import { 
  UserPlus, 
  Trash2, 
  CheckCircle, 
...
export default function RecruiterIntakeView({
  ...
  const activeSupplyCompany = useMemo(() => {
    return rows[0]?.supply_company || companies[0]?.name || "";
  }, [rows, companies]);
  ...
  const handleAddRow = () => {
    ...
    setRows([
      ...
      { 
        ...
        supply_company: lastRow?.supply_company || companies[0]?.name || "" 
      }
    ]);
  };
  ...
  const handleRemoveRow = (index: number) => {
    if (rows.length === 1) {
      setRows([{ name: "", passport: "", category: categories[0]?.name || "", supply_company: companies[0]?.name || "" }]);
    } else {
        ...
        lines.forEach((lineVals, offset) => {
          ...
          if (targetIndex < newRows.length) {
            ...
            newRows.push({
              ...
              supply_company: rows[rows.length - 1]?.supply_company || companies[0]?.name || ""
            });
          }
        });
        setRows(newRows);
  ...
  const bureauQueue = useMemo(() => {
    return workers.filter((w) => {
      ...
      // Filter strictly by the related recruited company currently active in the intake form
      if (w.supply_company !== activeSupplyCompany) {
        return false;
      }
                        ...
                        <td className="p-1.5 relative">
                          <input
                            ...
                            className={`w-full bg-paper/20 border rounded px-2 py-1.5 text-xs outline-none mono-text font-medium ${
                              isDuplicateInDb || isDuplicateInBatch ? "border-bad text-bad bg-red-50/20" : "border-line/60 focus:border-accent"
                            }`}
                            required
                          />
        ...
        <div className="xl:col-span-5 bg-card border border-line rounded-xl p-5 shadow-sm space-y-4 flex flex-col justify-between max-h-screen">
          <div className="space-y-3">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 p-2.5 bg-paper/40 rounded-lg border border-line/60">
              ...
              />
              <div className="text-[11px] px-2.5 py-1.5 bg-accent/5 border border-accent/20 rounded text-accent font-semibold truncate flex items-center justify-center gap-1">
                <Building className="w-3.5 h-3.5 shrink-0" />
                <span>{activeSupplyCompany}</span>
              </div>
            </div>
          </div>
                    ...
                    <div className="flex sm:flex-col items-stretch gap-1.5 w-full sm:w-auto shrink-0">
                      <div className="flex gap-1">
                        <button
                          ...
                          className={`flex-1 px-2.5 py-1 text-[10px] uppercase font-mono rounded tracking-tight transition-all cursor-pointer border ${
                            isCleared 
